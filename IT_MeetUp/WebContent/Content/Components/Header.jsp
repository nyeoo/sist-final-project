...
<nav class="navbar navbar-expand-md m-header-area sticky-top"
	...
	<div class="container-xl">
		...
		<div class="offcanvas offcanvas-end text-bg-dark m-right-nav"
			id="navbarsExample05">
			<input type="hidden" id="sessionScopeadmin"
				value="${sessionScope.admin}">
				...
				<c:choose>
					...
					<li class="nav-item dropdown"><a
						...
						<ul class="dropdown-menu">
							...
							<li><a class="dropdown-item" href="completeproject.action">완료된
									프로젝트</a></li>
						</ul></li>
					<li class="nav-item"><input type="hidden"
						id="sessionScopePiMemCode"
						value="${sessionScope.loginDTO.piMemCode}"> <input
						type="hidden" id="opCode" value="${sessionScope.opCode}">
						<input type="hidden" id="leaveTeamPcCodes"
						value="${sessionScope.leaveTeamPcCodes}"> <%-- <a class="nav-link" href="grouphome.action?memCode=${sessionScope.loginDTO.piMemCode}">Meet 그룹</a> --%>
						<a class="nav-link" id="groupHomeLink" href="javascript:;">Meet
							그룹</a></li>
				</ul>
					...
					<li class="nav-item dropdown list-group"><a
						...
						<div class="dropdown-menu dropdown-menu-end notifications"
							...
							<div class="tab-content" id="nav-management-subtabContent">
										...
										<div class="tab-pane fade show active" id="nav-applied"
											...
											<div class="tab-content"
														...
														<c:forEach var="pickNtfItem" items="${pickNtfList }">
															<div class="alert" role="alert">
																<strong> <span class="badge text-bg-primary">픽
																	...
																	${pickNtfItem.pickDate } </span>
																<hr class="my-2">
																<p>
																	<strong> ["${pickNtfItem.job }"] 로 신청하신
																		["${pickNtfItem.title }"] 팀 개설 요청에 픽!되었습니다. </strong> (픽된 멤버로
																	프로젝트가 만들어집니다. 신청을 취소하실 경우 신고처리가 부여됩니다.) <br> <a
																		href="#" class="alert-link"> 더보기 <i
												...
												<div class="tab-pane fade" id="participant-confirmation"
													...
													<c:forEach var="partCheckNtfItem"
														items="${partCheckNtfList }">
														<div class="alert" role="alert">
															<strong> <span class="badge text-bg-secondary">
																	참여자 확인 알림 </span>
															</strong> <span class="text-muted float-end small">
																${partCheckNtfItem.pcDate } </span>
															<hr class="my-2">
															<p>
																<strong> ["${partCheckNtfItem.job }"] 로 신청하신
																	["${partCheckNtfItem.title }"] 팀 개설 요청 그룹이 개설될 예정입니다.
																	프로젝트의 내용을 확인해주세요. </strong>
															</p>
															<p>
																(3일안에 확인하지 않으실 경우, 프로젝트가 실패됩니다.) <a href="#"
																	class="alert-link"> 더보기 <i
																	class="bi bi-plus-square"></i>
																</a>
															</p>
														</div>
													...
													<c:forEach var="ddayNtfItem" items="${ddayNtfList }">
														<div class="alert" role="alert">
															<strong> <span class="badge text-bg-success">
																...
																${ddayNtfItem.dnDate } </span>
															<hr class="my-2">
															<p>
																<!-- <strong>회원님이 선호하는 "(직무)", "(모임방식)", "(지역)" 에 딱 맞는 팀 개설 요청입니다.</strong> -->
																<strong> 프로젝트 ["${ddayNtfItem.title }"] 에 딱 맞는
																	팀 개설 요청입니다.</strong>
															</p>
															<p>
																(게시물을 보실려면 플러스 버튼을 누르세요.) <a href="#" class="alert-link">
												...
												<div class="tab-pane fade" id="comment-alert"
													...
													<c:forEach var="commentNtfItem" items="${commentNtfList }">
														<div class="alert" role="alert">
															<strong> <span class="badge text-bg-danger">
																	댓글알림 </span>
															</strong> <span class="text-muted float-end small">
																${commentNtfItem.comDate } </span>
															<hr class="my-2">
															<p>
																<strong> 작성한 ["${commentNtfItem.title }"] 프로젝트
																	게시물에 댓글이 작성되었습니다.</strong>
															</p>
															<p>
																(댓글을 확인하실려면 플러스 버튼을 누르세요.) <a href="#"
																	class="alert-link"> 더보기 <i
																	class="bi bi-plus-square"></i>
																</a>
															</p>
												...
												<div class="tab-pane fade" id="reply-alert" role="tabpanel"
													...
													<c:forEach var="recommentNtfItem"
														items="${recommentNtfList }">
														<div class="alert" role="alert">
															<strong> <span class="badge text-bg-warning">대댓글알림</span>
															</strong> <span class="text-muted float-end small">
																${recommentNtfItem.rnDate } </span>
															<hr class="my-2">
															<p>
																<strong>회원님이 작성한 댓글에 답변이 작성되었습니다.</strong>
															</p>
															<p>
																(확인하실려면 플러스 버튼을 누르세요.) <a href="#" class="alert-link">
																	더보기 <i class="bi bi-plus-square"></i>
																</a>
															</p>
														</div>
								...
								<div class="tab-content">
									<!-- 업무 미할당 -->
									<div class="tab-pane fade show active" id="task-alert"
										role="tabpanel" aria-labelledby="task-alert-tab">
										<!-- 업무 미할당 알림 내용 -->
									</div>
									<!-- 업무 보고 누락 -->
									<div class="tab-pane fade" id="report-missing-alert"
										role="tabpanel" aria-labelledby="report-missing-alert-tab">
										<!-- 업무 보고 누락 알림 내용 -->
									</div>
									<!-- 주간업무보고 미작성 -->
									<div class="tab-pane fade" id="weekly-report-missing-alert"
										role="tabpanel"
										aria-labelledby="weekly-report-missing-alert-tab">
										<!-- 주간업무보고 미작성 알림 내용 -->
									</div>
									<!-- 평가 미 작성 -->
							...
							<div class="flex-shrink-0 dropdown m-avator">
								...
								<ul class="dropdown-menu dropdown-menu-end text-small shadow">
									...
									<li><a class="dropdown-item"
										href="projectmanagement.action">모집/신청 관리</a></li>
									<li><a class="dropdown-item"
										href="namecard.action?memCode=${sessionScope.loginDTO.piMemCode}">네임카드</a></li>
									<li>
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			<div class="modal-header">
				<div class="modal-title fs-5 h1" id="modifyModalLabel">진행중 그룹
					없음안내</div>
			</div>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary"
					id="btn-myinfo-confirm" data-bs-dismiss="modal">확인</button>
			</div>
		</div>
	</div>
</div> -->
